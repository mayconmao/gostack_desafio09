...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const findProductsWithNoQuantityAvaliable = products.filter(
      product =>
        existentProducts.filter(pro => pro.id === product.id)[0].quantity <
        product.quantity,
    );
    ...
    const serializedProducts = products.map(product => ({
      ...
      price: existentProducts.filter(pro => pro.id === product.id)[0].price,
    }));
    ...
    const orderdProductsQuantity = order_products.map(product => ({
      id: product.product_id,
      quantity:
        existentProducts.filter(pro => pro.id === product.product_id)[0]
          .quantity - product.quantity,
    }));
